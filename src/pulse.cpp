...
Pulse::Pulse(const std::string& device, const size_t FS, const size_t nsamples,
             const bool stereo):buf(new int16_t[nsamples]) {
    samples = nsamples;
    ...
    int error;
    stream = pa_simple_new(nullptr, "glfwPI", PA_STREAM_RECORD,
                           default_device.c_str(), "glfwPI monitor",
                           &sample_spec, nullptr, &buffer_attr, &error);
